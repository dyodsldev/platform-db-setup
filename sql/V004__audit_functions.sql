...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_audit_log_table') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_audit_log_performed_at') THEN
        CREATE INDEX idx_audit_log_performed_at ON audit.audit_log(performed_at DESC);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_audit_log_username') THEN
        CREATE INDEX idx_audit_log_username ON audit.audit_log(username);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_audit_log_operation') THEN
...
-- Usage: SELECT * FROM audit.get_audit_history('marts', 'patients', 'uuid-here');
-- ----------------------------------------------------------------
